...
(function () {
  ...
  // Get style from local storage or use the current HTML theme.
  let storedStyle =
    localStorage.getItem('templateCustomizer-' + templateName + '--Theme') ||
    (window.templateCustomizer?.settings?.defaultTheme ?? 'semidark');
  let currentStyle =
    storedStyle === 'system'
      ? window.matchMedia('(prefers-color-scheme: dark)').matches
        ? 'dark'
        : 'light'
      : storedStyle === 'semidark'
        ? 'light'
        : storedStyle;

  // Handle semidark mode initialization
  if (storedStyle === 'semidark') {
    ...
    window.Helpers.switchImage(currentStyle);
    // Update light/dark image based on current style
    window.Helpers.setTheme(storedStyle);
    document.documentElement.removeAttribute('data-semidark-menu');
    document.querySelector('#layout-menu')?.removeAttribute('data-bs-theme');
  }

  // Restore stored primary color on page load
  (function restorePrimaryColor() {
    const storedColor = getStoredPrimaryColor();
    if (storedColor) {
      window.Helpers.setColor(storedColor, true);
    }
  })();

  // Primary color helpers
  function getStoredPrimaryColor() {
    return localStorage.getItem('templateCustomizer-' + templateName + '--Color');
  }
  function applyPrimaryColor(color) {
    localStorage.setItem('templateCustomizer-' + templateName + '--Color', color);
    window.Helpers.setColor(color, true);
  }

  window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', () => {
    const storedTheme = window.Helpers.getStoredTheme();
    if (storedTheme !== 'light' && storedTheme !== 'dark' && storedTheme !== 'semidark') {
  ...
  window.addEventListener('DOMContentLoaded', () => {
    window.Helpers.showActiveTheme(storedStyle);
    getScrollbarWidth();
    ...
    document.querySelectorAll('[data-primary-color]').forEach(btn => {
      btn.addEventListener('click', () => {
        const color = btn.getAttribute('data-primary-color');
        applyPrimaryColor(color);
        document.querySelectorAll('[data-primary-color]').forEach(el => {
          el.querySelector('.color-check')?.classList.add('d-none');
        });
        btn.querySelector('.color-check')?.classList.remove('d-none');
      });
    });
    // Mark active stored color on load
    (function markActiveColor() {
      const storedColor = localStorage.getItem('templateCustomizer-' + templateName + '--Color');
      if (storedColor) {
        document.querySelectorAll('[data-primary-color]').forEach(el => {
          if (el.getAttribute('data-primary-color') === storedColor) {
            el.querySelector('.color-check')?.classList.remove('d-none');
          }
        });
      }
    })();
    ...
    if (typeof Pickr !== 'undefined') {
      const pickerEl = document.getElementById('custom-color-picker');
      const pickr = new Pickr({
        el: pickerEl,
        theme: 'nano',
        default: getStoredPrimaryColor() || '#7367f0',
        position: 'bottom-end',
        components: {
          preview: true,
          hue: true,
      ...
      document.getElementById('custom-color-trigger')?.addEventListener('click', (e) => {
        e.preventDefault();
        pickr.show();
      });
      pickr.on('save', (color) => {
